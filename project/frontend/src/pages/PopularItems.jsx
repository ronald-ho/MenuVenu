...
function PopularItems () {
  ...
  return (
    <Box sx={{
      ...
      <TextField sx={{ margin: '10px' }} label='Search items' value={search}
        onChange={(e) => setSearch(e.target.value)}/>
      <TextField sx={{ margin: '10px' }} select label='Pick a category' value={category}
        onChange={(e) => setCategory(e.target.value)}>
        <MenuItem value={0}>All Items</MenuItem>
        {categories.map((category) => <MenuItem key={category.category_id}
          value={category.category_id}>{category.name}</MenuItem>)}
      </TextField>
      <TextField sx={{ margin: '10px' }} select label='Pick a filter' value={filter}
      ...
      <div style={{
        ...
        {items && <Table>
          ...
          <TableBody>
            {items.map((item, index) => item.name.toLowerCase().includes(search.toLowerCase()) &&
              <TableRow key={index}>
                <TableCell component={Link}
                  to={'/itemstatistics/' + item.item_id}>{item.name}</TableCell>
                {filter === 'gross' &&
                  <TableCell>${item.gross_income ? item.gross_income.toFixed(2) : ''}</TableCell>}
                {filter === 'net' &&
                  <TableCell>${item.net_income ? item.net_income.toFixed(2) : ''}</TableCell>}
