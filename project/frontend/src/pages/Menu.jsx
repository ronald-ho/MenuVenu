...
function Menu ({ setmode }) {
  ...
  async function handleRequestBill () {
    ...
    if (data.order_count) {
      ...
      if (customerId) {
        ...
        const googleConnected = cust.customer_info.google_connected;
  ...
  return (
    <div style={{
      ...
      {showInfo && <Alert severity='info' aria-label='infoAlert'
        sx={{
          position: 'fixed',
          top: '17px',
          left: '500px',
      ...
      {showAlert && <Alert severity='error' aria-label='errorAlert' sx={{
        position: 'fixed',
        top: '17px',
        left: '500px',
        width: '300px'
