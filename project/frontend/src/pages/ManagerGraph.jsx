...
function ManagerGraph () {
  ...
  const fetchstats = async () => {
    ...
    for (const cat in category) {
      const splitted = category[cat].split(',')
      const result = await apiCall('manager/profit_tracker?time=' + period + '&filter=' + type + '&category_id=' + splitted[0], 'GET', {});
  ...
  return (
    <Box sx={{ textAlign: 'center' }}>
      <Toolbar sx={{
        ...
        <TextField select label='Select statistic' value={type} onChange={(e) => {
        ...
        <TextField select label='Select period' value={period} onChange={(e) => {
        ...
        <TextField select label='Select filter' SelectProps={{
          ...
          {categories.map((category) => (
            <MenuItem key={category.category_id}
              value={`${category.category_id},${category.name}`}>{category.name}</MenuItem>
          ))}
        </TextField>
      </Toolbar>
